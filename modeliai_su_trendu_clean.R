...
make_month_trend <- function(time_vec, origin_time) {
  
  time_month <- lubridate::floor_date(time_vec, unit = "month")
  origin_month <- lubridate::floor_date(origin_time, unit = "month")
  
  trend <- 
    (lubridate::year(time_month) - lubridate::year(origin_month)) * 12 +
    (lubridate::month(time_month) - lubridate::month(origin_month))
  
  as.numeric(trend)
}

make_sarimax_xreg <- function(time_vec, origin_time) {
  dt <- data.table(hour_time = time_vec)
  
  dt[, trend := make_month_trend(hour_time, origin_time)]
  
  dt[, weekday := factor(
    lubridate::wday(hour_time, week_start = 1, label = FALSE),
    levels = 1:7,
    labels = c(
      "pirmadienis",
      "antradienis",
  ...
  X <- model.matrix(~ trend + weekday, data = dt)[, -1, drop = FALSE]
  X <- as.matrix(X)
...
make_glarma_xreg <- function(dt, origin_time) {
  dt <- copy(dt)
  ...
  X <- model.matrix(~ trend + hour + weekday, data = dt)
  X <- as.matrix(X)
  storage.mode(X) <- "double"
...
origin_time <- min(one$hour_time)

xreg <- make_sarimax_xreg(one$hour_time, origin_time)
...
fit_one_seniunija_file <- function(i) {
  ...
  origin_time <- min(time_train)
  
  xreg_train <- make_sarimax_xreg(time_train, origin_time)
  xreg_test <- make_sarimax_xreg(time_test, origin_time)
  ...
  if (fit_mode == "fixed") {
    ...
    model_name <- sprintf(
      "ARIMA(%d,%d,%d)(%d,%d,%d)[%d] + trend + weekday dummy",
...
saveRDS(
  sarima_metrics,
  "sarima_metrics_weekday_dummy_trend.rds",
  compress = "gzip"
)

saveRDS(
  sarima_predictions,
  "sarima_predictions_weekday_dummy_trend.rds",
  compress = "gzip"
)

cat("Rezultatai išsaugoti:\n")
cat("- sarima_metrics_weekday_dummy_trend.rds\n")
cat("- sarima_predictions_weekday_dummy_trend.rds\n\n")
...
if (!exists("sarima_metrics")) {
  sarima_metrics <- readRDS("sarima_metrics_weekday_dummy_trend.rds")
  setDT(sarima_metrics)
}

if (!exists("sarima_predictions")) {
  sarima_predictions <- readRDS("sarima_predictions_weekday_dummy_trend.rds")
...
saveRDS(
  sarima_predictions,
  "sarima_predictions_weekday_dummy_trend_rounded.rds",
  compress = "gzip"
)

saveRDS(
  sarima_metrics,
  "sarima_metrics_weekday_dummy_trend_rounded.rds",
  compress = "gzip"
)

tt <- summary(fit_manual)

#trend


extract_sarimax_trend_coef <- function(sid, model_dir = "models_sarima_trend") {
  ...
  if (!"trend" %in% names(coefs)) {
    return(data.table(
      sen_kodas = sid,
      status = "trend_not_found",
      trend_coef = NA_real_,
      trend_se = NA_real_,
      trend_z = NA_real_,
      trend_p_value = NA_real_,
  ...
  trend_coef <- as.numeric(coefs["trend"])
  ...
  trend_var <- tryCatch({
    ...
    if (
      !is.null(vc) &&
      "trend" %in% rownames(vc) &&
      "trend" %in% colnames(vc)
    ) {
      as.numeric(vc["trend", "trend"])
    } else {
      NA_real_
    }
  }, error = function(e) NA_real_)
  ...
  trend_se <- ifelse(
    is.finite(trend_var) && trend_var >= 0,
    sqrt(trend_var),
    NA_real_
  )
  
  trend_z <- ifelse(
    is.finite(trend_se) && trend_se > 0,
    trend_coef / trend_se,
    NA_real_
  )
  
  trend_p_value <- ifelse(
    is.finite(trend_z),
    2 * pnorm(abs(trend_z), lower.tail = FALSE),
    NA_real_
  )
  ...
  data.table(
    ...
    trend_per_year = 12 * trend_coef
  )
}

model_dir <- "models_sarima_trend"

sids <- sort(unique(as.character(result$sen_kodas)))

sarimax_trend_coefs <- rbindlist(
  lapply(
    sids,
    extract_sarimax_trend_coef,
    model_dir = model_dir
  ),
  fill = TRUE
)

sarimax_trend_coefs[, abs_trend_coef := abs(trend_coef)]

sarimax_trend_coefs[, trend_direction := fifelse(
  is.na(trend_coef),
  NA_character_,
  fifelse(
    trend_coef > 0,
    "Augantis trendas",
    fifelse(trend_coef < 0, "Mažėjantis trendas", "Be trendo")
  )
)]

sarimax_trend_coefs[, significant_5pct := fifelse(
  is.na(trend_p_value),
  NA,
  trend_p_value < 0.05
)]

setorder(sarimax_trend_coefs, -abs_trend_coef)

print(sarimax_trend_coefs)

sarimax_trend_coefs[, sen_kodas := as.character(sen_kodas)]
seniunijos[, SEN_KODAS := as.character(SEN_KODAS)]

seniunijos_names <- copy(seniunijos)

if ("SEN_NUO" %in% names(seniunijos_names)) {
  seniunijos_names[, SEN_NUO_tmp := as.IDate(SEN_NUO)]
  setorderv(
    seniunijos_names,
    cols = c("SEN_KODAS", "SEN_NUO_tmp"),
    order = c(1, -1),
    na.last = TRUE
  )
}

seniunijos_names <- seniunijos_names[
  !is.na(SEN_KODAS) & !is.na(VARDAS_K),
  .SD[1],
  by = SEN_KODAS
][
  ,
...
format_number_comma <- function(x, digits = 4) {
  ifelse(
    is.na(x),
    "",
    formatC(x, format = "f", digits = digits, decimal.mark = ",")
...
sarimax_trend_table[is.na(seniunija), seniunija := sen_kodas]

sarimax_trend_table <- sarimax_trend_table[
  ,
  .(
    seniunija,
    trend_coef = format_number_comma(trend_coef, 4),
    p_value = format_p_value(trend_p_value)
  )
]

setorder(sarimax_trend_table, seniunija)
...
trend_latex <- kable(
  ...
  booktabs = TRUE,
  escape = FALSE,
  caption = "SARIMAX modelių trendo koeficientai pagal seniūnijas",
  label = "tab:sarimax_trend_coefs",
  col.names = c(
    "Seniūnija",
    "Trendo koeficientas",
    "p reikšmė"
...
save_kable(
  trend_latex,
  file = "tables/sarimax_trend_coefs.tex"
)
...
result[, sen_kodas := as.character(sen_kodas)]
sarima_predictions[, sen_kodas := as.character(sen_kodas)]
seniunijos[, SEN_KODAS := as.character(SEN_KODAS)]
...
get_sen_name <- function(sid) {
  sid <- as.character(sid)
  name_i <- seniunijos_names[sen_kodas == sid, vardas_k][1]
  
  if (length(name_i) == 0 || is.na(name_i) || name_i == "") {
    return(sid)
  }
...
if (!exists("make_count_pred")) {
  make_count_pred <- function(x) {
    pmax(0, round(as.numeric(x)))
  }
}

chosen_sids <- sort(unique(result[!is.na(sen_kodas), sen_kodas]))
...
make_test_forecast_plot_data <- function(sid) {
  ...
  missing_cols <- setdiff(colnames(xreg_train), colnames(xreg_test))
  ...
  if (length(missing_cols) > 0) {
    ...
    xreg_test <- cbind(xreg_test, add_mat)
  }
...
plot_dt <- if (length(plot_dt_list) > 0) {
  rbindlist(plot_dt_list, fill = TRUE)
} else {
  data.table()
}
...
plot_dt_snaive_list <- Filter(
  Negate(is.null),
  lapply(chosen_sids, function(sid) {
...
plot_dt_snaive <- if (length(plot_dt_snaive_list) > 0) {
  rbindlist(plot_dt_snaive_list, fill = TRUE)
} else {
  data.table()
}

snaive_sids <- if (nrow(plot_dt_snaive) > 0) {
  sort(unique(plot_dt_snaive$sen_kodas))
} else {
  character()
}

plots_by_sid_snaive <- lapply(snaive_sids, function(sid) {
  ...
  place_name <- get_sen_name(sid)
  
  graph_title <- paste0(
    "Sezoninė naivioji prognozė ",
    place_name,
    " seniūnijoje"
  )
...
names(plots_by_sid_snaive) <- snaive_sids
...
for (sid in names(plots_by_sid_snaive)) {
  ...
  ggsave(
    filename = paste0("images/snaive_forecast_7_days_", safe_id(sid), ".png"),
    plot = p,
    width = 12,
    height = 7,
    dpi = 300
...
sarimax_sids <- if (nrow(plot_dt) > 0) {
  sort(unique(plot_dt$sen_kodas))
} else {
  character()
}

plots_by_sid <- lapply(sarimax_sids, function(sid) {
  ...
  ggplot(dt_i, aes(x = hour_time)) +
    geom_ribbon(
      ...
      aes(
        ymin = lower_95,
        ymax = upper_95,
        fill = "95% prognozavimo intervalas"
      ),
      alpha = 0.20
    ) +
    geom_ribbon(
      ...
      aes(
        ymin = lower_80,
        ymax = upper_80,
        fill = "80% prognozavimo intervalas"
      ),
      alpha = 0.35
    ) +
    geom_line(
    ...
    scale_fill_manual(
      values = c(
        "95% prognozavimo intervalas" = "#4B0082",
        "80% prognozavimo intervalas" = "#FF8C00"
      )
    ) +
    theme(
      legend.position = "bottom"
...
names(plots_by_sid) <- sarimax_sids
...
for (sid in names(plots_by_sid)) {
  ...
  ggsave(
    filename = paste0("images/sarima_forecast_", safe_id(sid), ".png"),
    plot = p,
    width = 12,
    height = 7,
    dpi = 300
...
origin_time <- min(train$hour_time)

X_train <- make_glarma_xreg(train, origin_time)
...
model_dir <- "models_glarma_trend"
pred_dir <- "predictions_glarma_trend"
...
fit_one_glarma_task <- function(task_i) {
  ...
  X_train <- make_glarma_xreg(train, origin_time)
  X_test <- make_glarma_xreg(test, origin_time)
...
saveRDS(
  glarma_metrics,
  "glarma_metrics_p3_q0_trend_all_seniunijos.rds",
  compress = "gzip"
)

saveRDS(
  glarma_predictions,
  "glarma_predictions_p3_q0_trend_all_seniunijos.rds",
  compress = "gzip"
)

fwrite(
  glarma_metrics,
  "glarma_metrics_p3_q0_trend_all_seniunijos.csv"
)

cat("Rezultatai išsaugoti:\n")
cat("- glarma_metrics_p3_q0_trend_all_seniunijos.rds\n")
cat("- glarma_predictions_p3_q0_trend_all_seniunijos.rds\n")
cat("- glarma_metrics_p3_q0_trend_all_seniunijos.csv\n")
...
model_dir <- "models_glarma_trend"
bootstrap_dir <- "bootstrap_glarma_trend"
images_dir <- "images"
...
if (!exists("chosen_sids")) {
  chosen_sids <- sort(unique(result[!is.na(sen_kodas), sen_kodas]))
} else {
  chosen_sids <- as.character(chosen_sids)
}
...
plot_titles <- setNames(
  seniunijos_names$vardas_k,
  seniunijos_names$sen_kodas
)
...
compute_and_save_glarma_bootstrap <- function(
  ...
  seed_i <- 123 +
    sum(utf8ToInt(as.character(sid))) +
    ifelse(distr_i == "NegBin", 10000, 0)
...
plot_glarma_bootstrap <- function(
  ...
  sid <- as.character(boot_data$metadata$sen_kodas)
...
compute_one_bootstrap_worker <- function(sid) {
  ...
  if (file.exists(out_file) && !overwrite_bootstrap) {
    return(data.table(
      sen_kodas = sid,
      status = "exists",
      file = out_file,
  ...
  res <- tryCatch(
    {
      ...
      data.table(
        ...
        file = out_file,
        error_message = NA_character_,
        time_min = as.numeric(difftime(Sys.time(), t0, units = "mins"))
      )
    },
    error = function(e) {
      data.table(
...
cat(sprintf(
  "GLARMA bootstrap išlygiagretintas: %d procesai\n\n",
  n_cores
))
...
parallel::clusterExport(
  ...
  varlist = c(
    ...
    "compute_one_bootstrap_worker"
  ),
  envir = environment()
)

bootstrap_status <- tryCatch(
  {
    data.table::rbindlist(
      parallel::parLapplyLB(
        cl = cl,
        X = chosen_sids,
        fun = compute_one_bootstrap_worker
      ),
      fill = TRUE
    )
  },
...
setorder(bootstrap_status, sen_kodas)
...
saveRDS(
  ...
  file.path(
    bootstrap_dir,
    paste0("bootstrap_status_", distr_i, "_B", B, ".rds")
  ),
  compress = "gzip"
)

ok_files <- bootstrap_status[
  status %in% c("ok", "exists") & file.exists(file),
  .(sen_kodas, file)
]

for (i in seq_len(nrow(ok_files))) {
  
  sid <- ok_files$sen_kodas[i]
  boot_file <- ok_files$file[i]
  
  boot_data <- readRDS(boot_file)
  ...
  ggsave(
    filename = file.path(
      images_dir,
      paste0("glarma_bootstrap_forecast_", safe_id(sid), "_", distr_i, ".png")
    ),
    plot = p,
    width = 12,
    height = 7,
...
sarima_model_dir <- "models_sarima_trend"
glarma_model_dir <- "models_glarma_trend"
...
if (!exists("glarma_predictions")) {
  glarma_predictions <- readRDS("glarma_predictions_p3_q0_trend_all_seniunijos.rds")
}

if (!exists("glarma_metrics")) {
  glarma_metrics <- readRDS("glarma_metrics_p3_q0_trend_all_seniunijos.rds")
}

if (!exists("sarima_metrics")) {
  sarima_metrics <- readRDS("sarima_metrics_weekday_dummy_trend_rounded.rds")
}
...
saveRDS(
  coverage_long,
  "interval_coverage_95_long_trend.rds",
  compress = "gzip"
)

saveRDS(
  coverage_pct_table,
  "interval_coverage_95_pct_table_trend.rds",
  compress = "gzip"
)

fwrite(
  coverage_long,
  "interval_coverage_95_long_trend.csv"
)

fwrite(
  coverage_pct_table,
  "interval_coverage_95_pct_table_trend.csv"
)
...
save_kable(
  table_latex,
  file = "tables/interval_coverage_95_trend.tex"
)
...
images_scan_dir <- "images"
latex_images_dir <- "images/priedai"
tex_dir <- "tex"
out_tex_file <- file.path(tex_dir, "all_figures.tex")
use_graphicspath <- FALSE
...
get_sen_name <- function(sid) {
  sid <- as.character(sid)
  name_i <- sen_names[sen_kodas == sid, vardas_k][1]
...
extract_sid <- function(file) {
  file_base <- basename(file)
  sid <- regmatches(file_base, regexpr("[0-9]{4}", file_base))
...
make_caption <- function(file) {
  file_base <- basename(file)
  sid <- extract_sid(file_base)
  place_name <- get_sen_name(sid)
  
  if (grepl("^glarma_bootstrap_forecast_", file_base)) {
    caption <- paste0("GLARMA modelio prognozavimas ", place_name, " seniūnijoje")
  } else if (grepl("^sarima_forecast_", file_base)) {
    caption <- paste0("SARIMAX modelio prognozavimas ", place_name, " seniūnijoje")
  } else if (grepl("^snaive_forecast_7_days_", file_base)) {
    caption <- paste0("Sezoninis naivusis prognozavimas ", place_name, " seniūnijoje")
  } else {
    caption <- paste0("Prognozavimo rezultatai ", place_name, " seniūnijoje")
  }
  
  latex_escape(caption)
}

make_label <- function(file) {
  file_base <- basename(file)
  label <- tools::file_path_sans_ext(file_base)
  label <- gsub("[^A-Za-z0-9]+", "_", label)
  paste0("fig:", label)
}

make_latex_path <- function(file) {
  file_base <- basename(file)
  
  if (use_graphicspath) {
    latex_path <- file_base
  } else {
    latex_path <- file.path(latex_images_dir, file_base)
  }
...
image_files <- list.files(
  path = images_scan_dir,
  pattern = "\\.png$",
  full.names = FALSE,
  recursive = TRUE
)

image_files <- gsub("\\\\", "/", image_files)

image_files <- image_files[
  grepl(
    "sarima_forecast_|snaive_forecast_|glarma_bootstrap_forecast_",
...
if (length(image_files) == 0) {
  stop("Nerasta grafikų aplanke: ", images_scan_dir)
}

tex_lines <- character(0)

figures_per_batch <- 12

for (i in seq_along(image_files)) {
  
  file <- image_files[i]
  
  latex_path <- make_latex_path(file)
  ...
  tex_block <- c(
    "\\begin{figure}[!htbp]",
    "    \\centering",
    paste0("    \\includegraphics[width=0.95\\textwidth,height=0.34\\textheight,keepaspectratio]{", latex_path, "}"),
  ...
  tex_lines <- c(tex_lines, tex_block)
  
  if (i %% figures_per_batch == 0) {
    tex_lines <- c(tex_lines, "\\clearpage", "")
  }
}

writeLines(
  as.character(tex_lines),
  con = out_tex_file,
  useBytes = TRUE
)
cat("Sukurta:", out_tex_file, "\n")
cat("Grafikų skaičius:", length(image_files), "\n")
...
get_glarma_coef_table <- function(fit) {
  ...
  if (is.null(s$coefficients1)) {
    return(NULL)
  }
  
  coef_table <- as.data.frame(s$coefficients1)
  
  needed_cols <- c("Estimate", "Std.Error", "z-ratio", "Pr(>|z|)")
  
  if (!all(needed_cols %in% names(coef_table))) {
    return(NULL)
  }
...
extract_glarma_trend_coef_from_file <- function(model_file) {
  
  file_name <- basename(model_file)
  
  sid <- sub(
    "^glarma_(.*)_NegBin_p3_q0\\.rds$",
    "\\1",
    file_name
  )
  ...
  empty_result <- function(status) {
    data.table(
      ...
      trend_z = NA_real_,
      trend_p_value = NA_real_,
      trend_month_pct = NA_real_,
      trend_year_pct = NA_real_
    )
  }
  ...
  if (inherits(fit, "error")) {
    return(empty_result("read_error"))
  }
  
  coef_table <- get_glarma_coef_table(fit)
  
  if (is.null(coef_table)) {
    return(empty_result("coefficients1_not_found"))
  }
  
  if (!"trend" %in% rownames(coef_table)) {
    return(empty_result("trend_not_found"))
  }
  
  trend_coef <- as.numeric(coef_table["trend", "Estimate"])
  trend_se <- as.numeric(coef_table["trend", "Std.Error"])
  trend_z <- as.numeric(coef_table["trend", "z-ratio"])
  trend_p_value <- as.numeric(coef_table["trend", "Pr(>|z|)"])
  ...
  data.table(
    ...
    trend_z = trend_z,
    trend_p_value = trend_p_value,
    trend_month_pct = (exp(trend_coef) - 1) * 100,
    trend_year_pct = (exp(12 * trend_coef) - 1) * 100
  )
}
...
model_files <- list.files(
  model_dir,
  pattern = "^glarma_.*_NegBin_p3_q0\\.rds$",
  full.names = TRUE
)

glarma_trend_coefs <- rbindlist(
  lapply(model_files, extract_glarma_trend_coef_from_file),
  fill = TRUE
)

glarma_trend_coefs[, abs_trend_coef := abs(trend_coef)]

glarma_trend_coefs[, trend_direction := fifelse(
  is.na(trend_coef),
  NA_character_,
  fifelse(
    trend_coef > 0,
...
glarma_trend_coefs[, significant_5pct := fifelse(
  is.na(trend_p_value),
  NA,
  trend_p_value < 0.05
)]

setorder(glarma_trend_coefs, -abs_trend_coef)

glarma_trend_coefs[, sen_kodas := as.character(sen_kodas)]
seniunijos[, SEN_KODAS := as.character(SEN_KODAS)]
...
glarma_trend_table[is.na(seniunija), seniunija := sen_kodas]

glarma_trend_table <- glarma_trend_table[
  status == "ok",
  .(
    seniunija,
    trend_coef = format_number_comma(trend_coef, 4),
    trend_exp = format_number_comma(exp(trend_coef), 4),
    p_value = format_p_value(trend_p_value)
  )
]

setorder(glarma_trend_table, seniunija)

dir.create("tables", showWarnings = FALSE)

glarma_trend_latex <- kable(
  glarma_trend_table,
  format = "latex",
  booktabs = TRUE,
  escape = FALSE,
  caption = "GLARMA modelių trendo koeficientai pagal seniūnijas",
  label = "tab:glarma_trend_coefs",
  col.names = c(
    "Seniūnija",
    "Trendo koeficientas",
    "$\\exp(\\hat{\\beta})$",
...
save_kable(
  glarma_trend_latex,
  file = "tables/glarma_trend_coefs.tex"
)
